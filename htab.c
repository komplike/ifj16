	      ...
	      const char *value)
{
  ...
  if ((item = htab_lookup (t, class, name)) == NULL)
    {
      printf ("nenachadza sa v tabulke\n");
      return E_SEM_OTHERS;
    }
  if (item->func != NULL)
    {
      printf ("znova sa pridava value\n");
      return 99;
    }
...
void check1 (struct htab_listitem *l){
	if (l->declr == NDECLR){
		printf("nedeklarovana premenna %s.%s\n", l->class, l->name);
		error = E_SEM_PROG;
	}
}
/* ****************************************************************************
...
int main() {
	...
	char *i = "42";
	char *d = "4.5";
	htab_add(t, "Main",	"str", 	NDEF, K_STRING);
	htab_add(t, "b",	"dbl",	DEF, K_DOUBLE);
	htab_add_val(t, "b", "dbl", d);
	htab_add(t, "b", 	"str",	DEF, K_STRING);
	htab_add_val(t, "b", "str", "hello");
	htab_add(t, "c", 	"int", 	DEF, K_INT);
	htab_add_val(t, "c", "int", i);
	htab_add(t, "A", 	"F",	NDEF,K_INT);
	htab_add(t, "hmm", 	"g",	DEF, K_INT);
	htab_add_val(t, "hmm", "g", "55");
	htab_add(t, "c", 	"func",	DEF, FUNC);
	htab_add(t, "c", 	"fnc", 	DEF, FUNC);
